import requests, pycurl, cStringIO, os
from bs4 import BeautifulSoup

ADDRESS = 'https://www.teamrankings.com/ajax/league/v3/stats_controller.php'
...
DATES = []
MONTHS = {"September": "09", "October": "10", "November": "11", "December": "12", "January": "01", "Feburary": "02"}
...
def pycURL(url, postfields): #takes a url and a postfield string and returns the response
	...
	c.perform()
	return buf.getvalue()
# print pycURL('https://www.teamrankings.com/ajax/league/v3/stats_controller.php', 'type=team-detail&league=nfl&season_id=12&cat_type=4&view=stats_v1&is_previous=0&date=11%2F12%2F2008&stat_id=8')

# print pycURL("https://www.teamrankings.com/ajax/league/v3/stats_controller.php", "")

def gatherPages():
	for week in os.listdir("/Users/lselig/Desktop/footballProject2.0/schedules"): # for each week (excluding week 1)
		with open("/Users/lselig/Desktop/footballProject2.0/schedules/" + week, "r") as file:
			lines = file.readlines() # each index is a line from the file
			date = lines[0][:-1].split() #take the first line, strip off newline char, and split it on whitespace
			# date[0] => Month
			# date[1] => Day
			monthNumber = MONTHS[date[0]] # reformat it so it can be passed to the website
			day = date[1]

			if(day in DAYS): # need to reformat it if the day is in (1, 9]
				day = DAYS[day]

			year = week[:4] # get the year from the filename (first 4 characters)
			formatted = monthNumber + "%2F" + day + "%2F" + year # put it all together how they want it
			DATES.append(formatted)
			print monthNumber, day, year, formatted
	PARAMETERS = 'type=team-detail&league=nfl&season_id=12&cat_type=4&view=stats_v1&is_previous=0&date=11%2F12%2F2008&stat_id=8'

	for date in DATES: # 192
		for stat in STATIDS:
			print "Working on data for:", stat, "on", date
			seperated = PARAMETERS.split("&") # break the string apart
			seperated[6] = "date=" + date # change what we pass to the website
			seperated[7] = "stat_id=" + str(STATIDS[stat]) # change what we pass to the website
			PARAMETERS = "&".join(seperated) # glue it back together
			currPage = pycURL(ADDRESS, PARAMETERS) # make the request
			with open("/Users/lselig/Desktop/footballProject2.0/htmlPages/" + date + stat + ".html", "w+") as file: # save the html so we don't have to pummel the site more than we have to
				file.write(currPage)
